...
## 0. Updating from a previous version of this project?

This version adds a full Admin Panel (new sidebar, RFID/Camera/Gate/Sensor/Reports/Settings/Security
pages) and a couple of new backend fields. If you already have the project running:

1. Replace your whole `client` and `server` folders with the ones in this zip (or copy over the
   changed files listed below if you made your own edits you want to keep).
2. In `server`, run `npm install` again (adds new backend deps — actually none were added server-side, skip if unchanged).
3. In `client`, run `npm install` again — this pulls in new packages (`lucide-react`, `jspdf`,
   `jspdf-autotable`, `xlsx`) used by the new Admin Panel.
4. Update `server/.env` — set `ADMIN_EMAIL=admin@gmail.com` and `ADMIN_PASSWORD=admin@123` (or keep
   your existing admin email/password, login will work either way).
5. Run `npm run seed` again in `server` — this only creates an admin account if one with that exact
   email doesn't already exist, so it's safe to re-run.
...
```
✅ Admin created -> email: admin@gmail.com  password: admin@123
...
- **Login as Admin** using the credentials from the seed step:
  ...
  - Logging in with the admin account automatically takes you to a separate **Admin Panel** at `/admin/dashboard` with its own sidebar — the regular student/faculty dashboard is never shown to the admin account.
...
- Edit your profile under **Profile**

As an admin you get a full sidebar-based Admin Panel with:
- **Dashboard** — overview cards (users, slots, bookings, active vehicles, entries/exits), a booking-trend chart, and a live recent-activity feed
- **User Management** — search users, view full profile + booking history, block/unblock, delete
- **Parking Slot Management** — add/edit/delete slots, toggle status (available/occupied/reserved/maintenance)
- **Booking Management** — search/filter all bookings, mark completed/cancelled
- **RFID Management** — register/assign/replace/deactivate RFID cards per user, simulated scan history
- **Camera Monitoring** — placeholder live-feed UI + vehicle image history (ready to wire up to a real camera later)
- **Gate Management** — manual open/close controls with simulated servo status
- **Sensor Monitoring** — ESP32/IR sensor/RFID reader connection health (shows Offline until real hardware is connected)
- **Analytics** — daily/weekly/monthly booking trends, peak hours, most-used slots, status breakdown (Recharts)
- **Reports** — booking & usage reports, exportable to PDF and Excel
- **Notifications** — send an announcement to all/students/faculty, with send history
- **System Settings** — parking hours, booking duration rules, holidays, contact email
- **Security** — admin profile, change password, login history, full activity audit log

> Camera Monitoring, Gate Management, and Sensor Monitoring are UI-complete but hardware-independent —
> they show clearly-labeled demo data since no physical ESP32/camera/sensor is wired in. Everything else
> (Users, Slots, Bookings, RFID, Analytics, Reports, Notifications, Settings, Security) is fully wired
> to the real backend and database.
