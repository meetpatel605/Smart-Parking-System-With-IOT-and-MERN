...
router.get("/available", protect, async (req, res) => {
  try {
    ...
    const slots = allSlots.map((slot) => {
      const booked = bookedSlotIds.has(slot._id.toString());
      const actualStatus = slot.status;
      let status = "available";
      if (actualStatus === "maintenance") status = "maintenance";
      else if (booked) status = "occupied";
      else if (actualStatus === "occupied" || actualStatus === "reserved") status = actualStatus;
      return {
        ...slot.toObject(),
        status,
      };
